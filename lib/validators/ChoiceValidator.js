...
class ChoiceValidator extends Validator_1.Validator {
    ...
    validate(data) {
        ...
        if (data.length < this.options.min)
            throw new ValidatorError_1.ValidatorError(3, this.form.trans(this.options.minMessage));
        if (data.length > this.options.max)
            throw new ValidatorError_1.ValidatorError(4, this.form.trans(this.options.maxMessage));
        data.forEach((d, i, a) => {
            if (!validator_1.isIn(d, this.options.choices)) {
                if (this.options.multiple)
                    throw new ValidatorError_1.ValidatorError(2, this.form.trans(this.options.multipleMessage));
                else
                    throw new ValidatorError_1.ValidatorError(1, this.form.trans(this.options.message));
            }
        });
        if (this.options.multiple)
            return data;
